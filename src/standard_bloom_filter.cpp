...
class BasicBloomFilter : public IterativeBloomFilter {
    ...
    public:
        ...
        BasicBloomFilter(size_t size2, size_t nb_hash, size_t nb_threads = 1) : IterativeBloomFilter(size2, nb_hash, nb_threads), _hash_functions(nb_hash) {
            _nb_bits = (_size >> 3LU);
            _bloom_data.resize(_nb_bits, (uint8_t) 0);
        }

        void insert(const u_int64_t& item) override {
            for (size_t i = 0 ; i < _nb_hash; i++) {
                u_int64_t h = _hash_functions(item, i) & _nb_bits;
                _bloom_data[h >> 3LU] |= _bit_mask[h & 7LU];
            }
        }
        ...
        bool contains(const u_int64_t& item) override {
            for (size_t i = 0 ; i < _nb_hash; i++) {
                u_int64_t h = _hash_functions(item, i) & _nb_bits;
				if ((_bloom_data[h >> 3LU] & _bit_mask[h & 7LU]) == 0) {
                    return false;
                }
            }
            return true;
...
class SyncBasicBloomFilter : public BasicBloomFilter {
    ...
    public:
        ...
        void insert(const u_int64_t& item) override {
            for (size_t i = 0 ; i < _nb_hash; i++) {
                u_int64_t h = _hash_functions(item, i) & _nb_bits;
                __sync_fetch_and_or(_bloom_data.data() + (h >> 3LU), _bit_mask[h & 7LU]);
            }
        }
